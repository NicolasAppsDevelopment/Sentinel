...
final class StreamController extends AbstractController {
    ...
    public function getStream(int $id, UserInterface $user): StreamedResponse
    {
        return new StreamedResponse(function () use ($user, $id) {
            while (ob_get_level() > 0) { // Clear all output buffers
                ob_end_flush();
            }
            set_time_limit(0);
...
//            }
            ...
            $url = 'http://' . $cameraDevice->getIp() . ':81/stream';
            $boundary = '123456789000000000000987654321';

            $stream = fopen($url, 'rb');
            if (!$stream) {
                return $this->apiResponseService->error('Unable to open camera stream');
            }

            $buffer = '';
            while (!feof($stream)) {
                $buffer .= fread($stream, 4096);
                while (($start = strpos($buffer, $boundary)) !== false) {
                    $next = strpos($buffer, $boundary, $start + strlen($boundary));
                    if ($next === false) {
                        break;
                    }
